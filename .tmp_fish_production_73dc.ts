...
export const ProductionGenerator = {
  generate(
    ...
    let currentFishTable: FishData[] = FISH_TABLE;

    if (settingsSpecies && settingsSpecies.length > 0) {
      ...
      if (mapped.length > 0) currentFishTable = mapped;
    }
    const months = MONTHS;
    ...
    while (attempts < 300) {
      ...
      for (const fish of currentFishTable) {
        const totalKg =
      ...
      for (let i = 0; i < result.length; i++) {
        const fish = currentFishTable.find((f) => f.name === result[i].name)!;
      ...
      if (totalValue < targetMin) {
        for (const prod of result) {
          const fish = currentFishTable.find((f) => f.name === prod.name)!;
    ...
    if (!bestResult) {
      console.warn("N├úo foi poss├¡vel gerar produ├º├úo ideal, usando fallback.");
      bestResult = currentFishTable.map((fish) => {
    ...
    console.log(
      `Produ├º├úo Gerada (${gender}): Total R$ ${finalTotal.toFixed(2)}`,
    );
