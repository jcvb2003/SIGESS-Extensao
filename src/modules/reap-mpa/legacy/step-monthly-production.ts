...
export const Page3 = {
  ...
  execute: async (manager: IWorkflowManager) => {
    console.log("REAP: Executando Página 3 (legacy, Meses)...");
    State.currentPage = 3;
    ...
    await Page3.ensureInitialData(settings);
  ...
  getStartIndex: (months: NodeListOf<Element>) => {
    ...
    for (let idx = 0; idx < months.length; idx++) {
      ...
      for (const [key, value] of Object.entries(MONTHS_MAP)) {
        if (monthText.includes(key)) { accMonthIndex = value; break; }
      }
  ...
  fillNormalMonth: async (activeContent: HTMLElement, realMonthIndex: number, settings: any) => {
    ...
    if (diasInput) Utils.setReactInput(diasInput, String(State.daysMap[realMonthIndex]));
